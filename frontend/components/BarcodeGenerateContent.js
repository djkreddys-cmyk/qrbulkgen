...
export default function BarcodeGenerateContent({ mode = "single" }) {
  ...
  function renderBarcodePrintCard(item, key) {
      ...
      <div key={key} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h3 className="mt-2 text-2xl font-black text-slate-950">{item.productName || productName || "Product Name"}</h3>
            <p className="mt-1 text-sm text-slate-500">{item.sku || sku || "SKU"}</p>
          </div>
          <div className="rounded-full bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700">{itemFormat.value}</div>
        </div>
        <div
          className={`mt-6 overflow-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-inner ${itemMatrixMode ? "flex justify-center" : ""}`}
                  ...
                  {a4Layout.pages.map((page, pageIndex) => (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {page.map((item, index) => renderBarcodePrintCard(item, item.__printKey || `barcode-a4-${pageIndex}-${index}`))}
                        </div>
                      </div>
                    </div>
                  ))}
