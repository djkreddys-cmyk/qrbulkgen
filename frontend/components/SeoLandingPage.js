      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-20 px-6 py-16">
        ...
        <section className="grid gap-6 rounded-[2rem] bg-slate-950 px-8 py-10 text-white lg:grid-cols-3">
          <div>
            ...
            <h2 className="mt-3 text-3xl font-bold">Use one product from setup to delivery.</h2>
          </div>
          <p className="text-slate-300">
            Generator, job queue, worker processing, downloads, and public destination experiences all
            stay connected so teams are not moving between disconnected tools.
          </p>
          <div className="space-y-3 rounded-3xl bg-white/10 p-6 text-sm text-slate-200">
            <p>Internal links that help SEO and conversion:</p>
            <div className="flex flex-wrap gap-3">
              <Link href="/pricing" className="rounded-full bg-white/15 px-3 py-2 hover:bg-white/25">
                Pricing
              </Link>
              <Link href="/blog" className="rounded-full bg-white/15 px-3 py-2 hover:bg-white/25">
                Blog
              </Link>
